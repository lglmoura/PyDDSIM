...
from pydssim.network.protocol.dht.kademlia.tuplaspace.tupla_space import TupleSpacePeer 
...
class DHTViewer(gtk.DrawingArea):
    def __init__(self, peer, *args, **kwargs):
        ...
        self.comms = {}
        self.incomingComms = {}
        # poison the peer with our GUI hooks
        self.peer._protocol.__gui = self
        
        self.peer._protocol.__realSendRPC = self.peer._protocol.sendRPC
        self.peer._protocol.sendRPC = self.__guiSendRPC
    
        self.peer._protocol.__realDatagramReceived = self.peer._protocol.datagramReceived
        self.peer._protocol.datagramReceived = self.__guiDatagramReceived
        self.msgCounter = 0
        self.printMsgCount = False
       
        
    def __guiSendRPC(self, neighbor, method, args, rawResponse=False):
        #print 'sending'
        self.drawComms(neighbor.id, method)
        self.msgCounter += 1
        return self.peer._protocol.__realSendRPC(neighbor, method, args, rawResponse)
    
    def __guiDatagramReceived(self, datagram, address):
        msgPrimitive = self.peer._protocol._encoder.decode(datagram)
        
        message = self.peer._protocol._translator.fromPrimitive(msgPrimitive)
        ...
        if isinstance(message, ErrorMessage):
            ...
            msg = message.request
        self.drawIncomingComms(message.peerID, msg)
        return self.peer._protocol.__realDatagramReceived(datagram, address)
    ...
    def draw(self, cr, width, height):
        ...
        blips = []
        kbucket = {}
        for i in range(len(self.peer._routerTable._buckets)):
            for neighbor in self.peer._routerTable._buckets[i]._neighbors:    
                blips.append(neighbor)
                kbucket[neighbor.id] = i
        # ...and now circles for all the other peers
        if len(blips) == 0:
            spacing = 180
        else:
    ...
    def drawComms(self, neighborID, msg):
        if neighborID not in self.comms:
            self.comms[neighborID] = msg
            gobject.timeout_add(750, self.removeComm, neighborID)
            self.window.invalidate_rect(self.allocation, False)
    
    def drawIncomingComms(self, neighborID, msg):
        if neighborID not in self.incomingComms:
            self.incomingComms[neighborID] = msg
            gobject.timeout_add(750, self.removeIncomingComm, neighborID)
            self.window.invalidate_rect(self.allocation, False)
    
    def removeIncomingComm(self, neighborID):
        try:
            del self.incomingComms[neighborID]
        finally:
            self.window.invalidate_rect(self.allocation, False)
            return False
    
    def removeComm(self, neighborID):
        try:
            del self.comms[neighborID]
        finally:
            self.window.invalidate_rect(self.allocation, False)
            return False
...
class DHTViewerWindow(gtk.Window):
    def __init__(self, peer):
        ...
        ################# "DHT DHT"-specific stuff ####################
        #frame = gtk.Frame()
        #frame.set_label('Store keyword-searchable data in the DHT')
        #frame.show()
        #notebook.append_page(frame, gtk.Label('DHT Extensions'))
        ...
        frame = gtk.Frame()
        frame.set_label('DHT Extensions')
        frame.show()
        lowlevelVbox.pack_start(frame)
        ...
        ################# "DHT Tuple Space"-specific stuff ####################
        #frame = gtk.Frame()
        #frame.set_label('Store keyword-searchable data in the DHT')
        #frame.show()
        #notebook.append_page(frame, gtk.Label('DHT Extensions'))
        ...
        tupleSpaceVbox.show()
        #frame.add(entangledTabVbox)
        notebook.append_page(tupleSpaceVbox, gtk.Label('DHT Tuple Space'))
    ...
    def publishData(self, sender, nameFunc, valueFunc):
        ...
        df = self.peer.publishData(name, value)
    ...
    def storeValue(self, sender, keyFunc, valueFunc):
        ...
        def completed(result):
            self.viewer.printMsgCount = True
        df = self.peer.iterativeStore(hKey, value)
        df.addCallback(completed)
    ...
    def getValue(self, sender, entryKey, showFunc):
        ...
        df = self.peer.iterativeFindValue(hKey)
    ...
    def deleteValue(self, sender, keyFunc):
        ...
        def completed(result):
            self.viewer.printMsgCount = True
        df = self.peer.iterativeDelete(hKey)
        df.addCallback(completed)
    ...
    def searchForKeywords(self, sender, entryKeyword, showFunc):
        ...
        df = self.peer.searchForKeywords(keyword)
        df.addCallback(showValue)
        df.addErrback(error)
    ...
    def getTuple(self, sender, entryTemplate, showFunc, blocking):
        ...
        if blocking == True:
            df = self.peer.get(template)
        else:
            df = self.peer.getIfExists(template)
        df.addCallback(showValue)
        df.addErrback(error)
    ...
    def readTuple(self, sender, entryTemplate, showFunc, blocking):
        ...
        if blocking == True:
            df = self.peer.read(template)
        else:
            df = self.peer.readIfExists(template)
        df.addCallback(showValue)
        df.addErrback(error)
...
if __name__ == '__main__':
    ...
    peer = TupleSpacePeer( udpPort=int(sys.argv[1]) )
    
    window = DHTViewerWindow(peer)

    window.set_default_size(640, 640)
    window.set_title('Viewer - DHT on port %s' % sys.argv[1])

    window.present()
    
    peer.joinNetwork(knownNodes)
    peer.printNeighbors()
